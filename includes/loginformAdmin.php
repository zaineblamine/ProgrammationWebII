<?php
class LoginFormAdmin extends DbConn
{
    public function checkLogin($myusername, $mypassword)
    {
        ...
        $stmt = $db->conn->prepare("SELECT * FROM ".$tbl_admin." WHERE username = :myusername");
    ...
    public function insertAttempt($username)
    {
        try {
            $db = new DbConn;
            $conf = new GlobalConf;
            $tbl_admin_attempts = $db->tbl_admin_attempts;
            $ip_address = $conf->ip_address;
            $login_timeout = $conf->login_timeout;
            $max_attempts = $conf->max_attempts;
            ...
            $stmt = $db->conn->prepare("INSERT INTO ".$tbl_admin_attempts." (ip, attempts, lastlogin, username) values(:ip, 1, :lastlogin, :username)");
        ...
        try {
            ...
            if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
                ...
                if ($timeDiff >= $login_timeout) {
                    ...
                    $sql = "UPDATE ".$tbl_admin_attempts." SET attempts = :attempts, lastlogin = :lastlogin where ip = :ip and username = :username";
                    $curr_attempts = 1;
